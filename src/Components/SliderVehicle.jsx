  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col transition-all hover:shadow-xl group h-full">
      ...
      <div className="px-5 py-3 flex justify-between items-center">
        <h3 className="text-xl font-bold text-gray-900 truncate pr-2 uppercase">
          {vehicle.name}
        </h3>
        <span
...
const SliderVehicle = () => {
  ...
  if (loading)
    return (
      <div className="py-20 text-center text-yellow-500 font-bold uppercase tracking-widest">
        Loading Fleet...
      </div>
    );
  ...
  return (
    <section className="bg-white py-12 md:py-16 px-6 text-center overflow-hidden">
      <div className="max-w-6xl mx-auto mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="text-left">
          ...
          <p className="text-gray-500 max-w-xl mt-2 text-xs md:text-sm leading-relaxed font-medium">
            Premium quality rides curated for your comfort and safety. Choose
            your perfect match from our latest fleet.
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <button
            onClick={prevSlide}
            className="w-12 h-12 rounded-full border border-gray-200 flex items-center justify-center hover:bg-yellow-500 hover:text-white transition-all shadow-sm"
          >
            <FaChevronLeft />
          </button>
          <button
      ...
      <div className="flex justify-center gap-2 mt-8">
        {Array.from({
          length: Math.max(0, vehiclesData.length - visibleCount + 1),
        }).map((_, i) => (
          <button
            key={i}
